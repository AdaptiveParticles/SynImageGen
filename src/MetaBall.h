...
public:
    ...
    float calculate_potential(float x,float y,float z){
        //
...
void generate_metaball_template(Object_template& obj_template,int sample_rate,float real_size,float density = 1000000,float rad_ratio = 3.0/8.0){
    ...
    for (int i = 0; i < obj_template.true_object_distribution.y_num; i++) {
        for (int j = 0; j < obj_template.true_object_distribution.x_num; j++) {
            for (int k = 0; k < obj_template.true_object_distribution.z_num; k++) {

                for (int l = 0; l < num_meta_balls; ++l) {
                    potential_field(i,j,k) += balls[l].calculate_potential((float)i,(float)j,(float)k);
                }
    ...
    for (int i = 0; i < obj_template.true_object_distribution.y_num; i++) {
        for (int j = 0; j < obj_template.true_object_distribution.x_num; j++) {
            for (int k = 0; k < obj_template.true_object_distribution.z_num; k++) {
                ...
                if (potential_field(i,j,k) > threshold) {
                    obj_template.true_object_distribution(i,j,k) = 1*obj_template.max_sample;
                } else {
                    obj_template.true_object_distribution(i,j,k) = 0;
                }
            }
        }
    }
